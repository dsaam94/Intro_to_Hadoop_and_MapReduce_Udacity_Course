...
def mapper():
    ...
    for line in reader:
        ...
        if (len(line) == 6):
            key_users = 'A'
            int(user_ptr_id), key_users, reputation, gold, silver, bronze = line
            writer.writerow(line)
        else:
            key_node = 'B'
            int(node_id), key_node, title, tagnames, author_id, body, node_type, parent_id, abs_parent_id, added_at, score, state_string, last_edited_id, last_activity_by_id, last_activity_at,
            active_revision_id, extra, extra_ref_id, extra_count, marked= line
            writer.writerow(line[node_id,key_node, title, tagnames,  author_id,  node_type,  parent_id,  abs_parent_id,  added_at,
score])
